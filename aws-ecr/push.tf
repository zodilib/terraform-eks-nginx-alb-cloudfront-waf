...
data "external" "hash" {
  ...
  #program = ["hash.sh", var.source_path]
}
...
resource "null_resource" "push" {
  ...
  provisioner "local-exec" {
    command     = "${coalesce(var.push_script, "${path.module}/push.sh")} ${var.source_path} ${aws_ecr_repository.pccw-repository.repository_url} ${var.tag}"
    interpreter = ["bash", "-c"]
  }
}